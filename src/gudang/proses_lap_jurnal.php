...
if($action == "jurnalBarang"){
	...
	// cek transaksi
	$sqlCekTransaksi = "SELECT COUNT(*) AS cek FROM tb_jurnal_transaksi";
	...
	?>
<h3 align="center">Jurnal Barang (Mentah)</h3>
<div class="thumbnail">
	<div class="caption"> 
<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="tabelJurnalBarang">
	...
	$hasilJurnal = mysql_query("SELECT * FROM tb_jurnal_transaksi");
...
elseif ($action == "autoTahun"){
	$sqlAutoTahun = "SELECT DISTINCT(YEAR(tanggal)) AS tahun FROM tb_jurnal_transaksi";
...
elseif($action == "filter"){
	...
	$hasilFilterJurnal = mysql_query("SELECT * FROM tb_jurnal_transaksi 
								WHERE 
